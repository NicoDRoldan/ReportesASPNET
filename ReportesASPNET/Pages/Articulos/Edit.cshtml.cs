...
namespace ReportesASPNET.Pages.Articulos
{
...

    {
        ...
        public void OnGet()
        {
            ...
            string idString = Request.Query["id_Articulo"];
                    ...
                    using (SqlCommand sqlCommand = new SqlCommand(query, sqlConnection))
                    {
                        ...
                        using (SqlDataReader reader = sqlCommand.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                articuloInfo.fecha = reader.IsDBNull(5) ? null : reader.GetDateTime(5);
                            }
                        }
                    }
                }
        ...
        {
            articuloInfo.idArticulo = int.Parse(Request.Query["id_Articulo"]);
            articuloInfo.nombre = Request.Form["nombre"];
            articuloInfo.rubro = Request.Form["rubro"];
            articuloInfo.activo = Boolean.Parse(Request.Form["activo"]);
            articuloInfo.descripcion = Request.Form["descripcion"];
            articuloInfo.fecha = DateTime.Parse(Request.Form["fecha"]);
            ...
            try
            {
                ...
                using (SqlConnection sqlConnection = new SqlConnection(connectionString))
                {
                    ...
                    string query = "UPDATE Articulos" +
                        "SET nombre=@nombre, rubro=@rubro, acivo=@activo, descripcion=@descripcion, fecha=@fecha" +
                    ...
                    using (SqlCommand sqlCommand = new SqlCommand(query, sqlConnection))
                    {
                        ...
                        sqlCommand.Parameters.AddWithValue("@fecha", articuloInfo.fecha);
